...
function SectionKicker({ children, light = false }: { children: React.ReactNode; light?: boolean }) {
    return (
        <p className={`flex items-center gap-4 text-[10px] font-bold uppercase tracking-[0.28em] ${light ? "text-white/75" : "text-[var(--brand-slate-muted)]"}`}>
            <span className="h-px w-12 bg-[var(--brand-gold)]" aria-hidden="true" />
            {children}
        </p>
    );
}
...
export default function HomePage() {
    ...
    return (
        <div className="w-full overflow-hidden bg-white text-[var(--brand-forest)]">
            <div className="grid h-[100svh] min-h-[640px] w-full grid-cols-[minmax(0,1fr)] grid-rows-[minmax(0,1fr)_auto]">
                <section className="relative min-h-0 min-w-0 overflow-hidden" aria-labelledby="home-heading">
                    ...
                    <div className="home-hero-panel absolute inset-x-5 bottom-0 rounded-t-xl bg-white px-6 py-6 text-[var(--brand-forest)] sm:inset-x-auto sm:left-8 sm:w-[440px] lg:left-[clamp(1.5rem,2.6vw,2.5rem)] lg:w-[min(32vw,460px)] lg:min-w-[390px] lg:px-8 lg:py-7">
                        <h1
                            id="home-heading"
                        ...
                        <span className="home-hero-rule mt-5 block h-1 w-10 origin-left bg-[var(--brand-gold)]" aria-hidden="true" />
                        <p className="mt-3 max-w-[390px] text-[0.95rem] leading-[1.42] text-[var(--brand-body-muted)] lg:text-[clamp(0.95rem,1.2vw,1.08rem)]">
                            At HOVUCA, young people lead, collaborate and create solutions for stronger, more inclusive communities in Cameroon.
                        </p>
                        <Link
                            href="/projects"
                            data-home-action
                            className="group mt-5 inline-flex h-11 w-[210px] items-center justify-between rounded-full bg-[var(--brand-coral-dark)] px-4 text-sm font-semibold text-white transition-colors hover:bg-[var(--brand-coral-deep)] focus-visible:bg-[var(--brand-coral-deep)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-gold)]"
                        >
                    ...
                    <aside className="absolute right-0 top-[15.9%] hidden w-[124px] overflow-hidden rounded-l-xl bg-white text-[var(--brand-forest)] lg:block" aria-label="Explore HOVUCA evidence">
                        {evidenceLinks.map((item, index) => (
                            <Link
                                key={item.number}
                                href={item.href}
                                className="group relative flex h-[8.26vw] max-h-[124px] flex-col justify-center border-b border-[var(--brand-gold)]/65 px-5 last:border-b-0 focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--brand-gold)]"
                            >
                                {index === 0 && <span className="absolute inset-y-8 left-3 w-[3px] bg-[var(--brand-gold)]" aria-hidden="true" />}
                                <span
                                    className={`text-[1.35rem] leading-none ${index === 0 ? "text-[var(--brand-gold-dark)]" : "text-[var(--brand-slate-deep)]"}`}
                                >
                                    {item.number}
                                </span>
                                <span className="mt-2 text-xs font-semibold text-[var(--brand-forest)] transition-colors group-hover:text-[var(--brand-coral-dark)]">
                                    {item.label}
                                </span>
                            </Link>
                        ))}
                    ...
                    <span className="absolute bottom-[18px] right-[315px] hidden h-px w-8 bg-[var(--brand-gold)] md:block" aria-hidden="true" />
                    <p className="absolute bottom-1.5 right-[30px] hidden h-[19px] w-[285px] items-center border-b border-[var(--brand-forest)]/25 bg-[var(--brand-periwinkle)] px-1 text-[11px] font-normal text-white md:flex">
                        Centre Region, Cameroon · Community-led program
                    </p>
                </section>

                <section className="min-w-0 overflow-hidden border-y border-[var(--brand-forest)]/15 bg-white py-6">
                    <p className="mb-8 text-center text-[10px] font-bold uppercase tracking-[0.28em] text-[var(--brand-slate-muted)]">Partners in the work</p>
                    <DonorCarousel />
                </section>
            </div>
            ...
            <section className="home-deferred-section home-programs bg-white px-6 py-20 lg:px-12 lg:py-24" aria-labelledby="programs-heading" data-home-reveal="programs">
                <div className="mx-auto max-w-[1280px]">
                    <div className="home-program-heading grid gap-10 border-b border-[var(--brand-forest)]/25 pb-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
                        <SectionKicker>{programsArePreviews ? "Program previews" : "Where we work"}</SectionKicker>
                        <div>
                            <h2 id="programs-heading" className="max-w-3xl text-4xl font-bold leading-[1.02] tracking-[-0.03em] text-[var(--brand-forest)] md:text-6xl">
                                Programs shaped around real lives.
                            </h2>
                            <p className="mt-6 max-w-2xl text-base leading-7 text-[var(--brand-copy-muted)]">
                    ...
                    <div className="home-program-list grid lg:grid-cols-3">
                        {programsLoading && <p className="col-span-full py-16 text-center text-sm text-[var(--brand-copy-muted)]" role="status">Loading programs…</p>}
                        {programsError && (
                            <div className="col-span-full py-14 text-center" role="alert">
                                <p className="font-semibold">Programs could not be loaded.</p>
                                <button type="button" onClick={() => void refetchPrograms()} className="mt-4 min-h-11 rounded-full border border-[var(--brand-forest)] px-5 text-sm font-semibold">Try again</button>
                            </div>
                        )}
                        {programs.map((program, index) => (
                            <Link
                                ...
                                className="group flex min-h-[300px] flex-col border-b border-[var(--brand-forest)]/25 px-0 py-9 text-[var(--brand-forest)] focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[var(--brand-gold)] lg:border-b-0 lg:border-r lg:px-8 lg:last:border-r-0 lg:first:pl-0"
                            >
                                <span className="text-sm text-[var(--brand-coral-dark)]">0{index + 1}</span>
                                <h3 className="mt-auto max-w-sm text-3xl font-bold leading-[1.07] tracking-[-0.025em]">
                                    {program.title}
                                </h3>
                                <p className="mt-5 max-w-sm text-sm leading-6 text-[var(--brand-copy-soft)]">{program.excerpt}</p>
                                <span className="mt-8 inline-flex items-center gap-3 text-xs font-semibold text-[var(--brand-coral-dark)]">
                ...
                <div className="mx-auto max-w-[1280px]">
                    <div data-home-reveal="rise" className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
                        <div>
                            ...
                            <h2 id="stories-heading" className="mt-5 text-4xl font-bold leading-[1.02] tracking-[-0.03em] text-[var(--brand-forest)] md:text-6xl">
                                Voices from the work.
                            </h2>
                        </div>
                        <Link href="/blog" data-home-action className="inline-flex items-center gap-3 text-sm font-semibold text-[var(--brand-coral-dark)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-gold)]">
                    ...
                    <div className="mt-12 grid items-start gap-8 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] lg:gap-12">
                        {articlesLoading && <p className="lg:col-span-2 py-16 text-center text-sm text-[var(--brand-copy-muted)]" role="status">Loading stories…</p>}
                        {articlesError && (
                            <div className="lg:col-span-2 py-14 text-center" role="alert">
                                <p className="font-semibold">Stories could not be loaded.</p>
                                <button type="button" onClick={() => void refetchArticles()} className="mt-4 min-h-11 rounded-full border border-[var(--brand-forest)] px-5 text-sm font-semibold">Try again</button>
                            </div>
                        )}
                        {articles[0] && (
                            <article data-home-reveal="story-feature" className="home-story-feature group min-w-0">
                                <Link href={storiesArePreviews ? "/blog" : `/blog/${articles[0].slug}`} className="block focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-coral)]">
                                    <div className="home-story-feature-image relative aspect-[5/4] overflow-hidden rounded-2xl bg-[var(--brand-sand)] sm:aspect-[4/3]">
                                        <Image
                                        ...
                                        />
                                        <span className="absolute left-5 top-5 rounded-full bg-white/95 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--brand-forest)] shadow-sm">Featured story</span>
                                    </div>
                                    <div className="home-story-feature-copy relative -mt-16 ml-5 rounded-tl-2xl bg-white px-6 pb-2 pt-6 sm:-mt-24 sm:ml-12 sm:px-9 sm:pt-8">
                                        <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[var(--brand-coral-dark)]">{articles[0].category?.name || "HOVUCA story"}</p>
                                        <h3 className="mt-3 max-w-xl text-2xl font-bold leading-[1.1] tracking-[-0.025em] text-[var(--brand-forest)] sm:text-4xl">{articles[0].title}</h3>
                                        <p className="mt-4 max-w-xl line-clamp-3 text-sm leading-6 text-[var(--brand-copy-subtle)]">{articles[0].excerpt}</p>
                                        <span className="mt-6 inline-flex items-center gap-3 text-sm font-semibold text-[var(--brand-coral-dark)]">Read the story <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></span>
                                    </div>
                                </Link>
                            </article>
                        )}
                        <div className="flex min-w-0 flex-col gap-8 lg:gap-10">
                            {articles.slice(1).map((article) => (
                                <article key={article.id} data-home-reveal="story-side" className="home-story-side group min-w-0 border-t border-[var(--brand-gold)] pt-5">
                                    <Link href={storiesArePreviews ? "/blog" : `/blog/${article.slug}`} className="grid grid-cols-[0.8fr_1fr] items-start gap-0 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-coral)]">
                                        <div className="home-story-side-image relative z-0 aspect-[4/5] overflow-hidden rounded-xl bg-[var(--brand-sand)]">
                                            <Image
                                        ...
                                        <div className="relative z-10 -ml-8 mt-[20%] min-w-0 rounded-2xl bg-white px-6 py-5 sm:-ml-10 sm:px-7 sm:py-6">
                                            <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[var(--brand-coral-dark)]">{article.category?.name || "HOVUCA story"}</p>
                                            <h3 className="mt-3 text-2xl font-bold leading-[1.12] tracking-[-0.02em] text-[var(--brand-forest)]">{article.title}</h3>
                                            <p className="mt-3 line-clamp-3 text-sm leading-6 text-[var(--brand-copy-subtle)]">{article.excerpt}</p>
                                            <span className="mt-4 inline-flex items-center gap-2 text-xs font-semibold text-[var(--brand-coral-dark)]">Read story <ArrowRight className="size-4 transition-transform group-hover:translate-x-1" /></span>
                                        </div>
                                    </Link>
                                </article>
                            ))}
            ...
            <section className="home-deferred-section home-resources grid bg-[var(--brand-forest)] text-white lg:grid-cols-[0.78fr_1.22fr]" aria-labelledby="resources-heading" data-home-reveal="resources">
                <div className="home-resource-intro flex min-h-[440px] flex-col justify-between border-b border-white/20 px-6 py-14 lg:border-b-0 lg:border-r lg:px-12 lg:py-16">
                    ...
                    <div>
                        ...
                        <Link href="/documents" data-home-action className="mt-8 inline-flex items-center gap-3 text-sm font-semibold text-[var(--brand-gold-light)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-gold-light)]">
                ...
                <div className="home-resource-list divide-y divide-white/20">
                    ...
                    {displayedResources.map((resource, index) => (
                        <Link key={resource.id} href="/documents" className="group grid min-h-[150px] grid-cols-[auto_1fr_auto] items-center gap-6 px-6 py-7 transition-colors hover:bg-white/5 lg:px-12">
                            <span className="text-sm text-[var(--brand-gold-light)]">0{index + 1}</span>
                            <span>
                            ...
                            <span className="flex h-11 w-11 items-center justify-center rounded-lg border border-white/35 text-[var(--brand-gold-light)] transition-transform group-hover:translate-x-1" aria-hidden="true"><FileText className="h-5 w-5" /></span>
                        </Link>
                    ))}
                </div>
            </section>
            ...
            <section className="home-deferred-section bg-white px-6 py-16 lg:px-12 lg:py-20" aria-labelledby="approach-cta-heading" data-home-reveal="approach">
                <div className="home-approach mx-auto flex max-w-[1280px] flex-col gap-8 border-y border-[var(--brand-forest)]/20 py-12 md:flex-row md:items-center md:justify-between md:gap-16 lg:py-14">
                    <div className="max-w-2xl">
                        <h2 id="approach-cta-heading" className="text-3xl font-bold leading-[1.08] tracking-[-0.03em] text-[var(--brand-forest)] md:text-4xl">
                            Explore the thinking behind the work.
                        </h2>
                        <p className="mt-4 max-w-xl text-base leading-7 text-[var(--brand-copy-muted)]">
                            Learn more about HOVUCA’s approach to child protection, opportunity and community-led change.
                        </p>
                    </div>
                    <Link href="/about" data-home-action className="group inline-flex min-h-12 w-fit shrink-0 items-center gap-5 rounded-full bg-[var(--brand-forest)] px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-[var(--brand-teal-muted)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-forest)]">
            ...
            <section className="home-deferred-section home-involved grid overflow-hidden lg:grid-cols-2" aria-labelledby="involved-heading" data-home-reveal="involved">
                <div className="home-involved-panel home-involved-panel-start bg-[var(--brand-gold)] px-6 py-16 text-[var(--brand-forest)] lg:px-12 lg:py-20">
                    ...
                    <p className="mt-7 max-w-xl text-base leading-7 text-[var(--brand-teal-muted)]">Volunteer alongside programs rooted in local knowledge and shared responsibility.</p>
                    <Link href="/volunteers" data-home-action className="mt-9 inline-flex items-center gap-3 rounded-full border border-[var(--brand-forest)] px-6 py-3 text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-forest)]">Volunteer with us <ArrowRight className="h-4 w-4" /></Link>
                </div>
                <div className="home-involved-panel home-involved-panel-end bg-[var(--brand-coral-dark)] px-6 py-16 text-white lg:px-12 lg:py-20">
            ...
            <section className="home-deferred-section bg-white px-6 py-20 lg:px-10 lg:py-24">
                <div className="mx-auto grid max-w-[1280px] gap-10 border-t border-[var(--brand-forest)] pt-10 lg:grid-cols-[1fr_0.85fr] lg:items-end">
                    <div>
                        <SectionKicker>Stay connected</SectionKicker>
                        <h2 className="mt-6 max-w-3xl text-3xl font-bold leading-[1.05] tracking-[-0.03em] text-[var(--brand-forest)] md:text-5xl">
                    ...
                    <nav aria-label="Follow HOVUCA on social media" className="text-center lg:col-span-2">
                        <p className="mb-3 text-sm font-semibold text-[var(--brand-forest)]">Keep up with us on social media</p>
