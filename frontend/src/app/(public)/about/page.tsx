...
export default function AboutPage() {
    return <div className="bg-white text-[#183b35]">
        <section className="grid lg:grid-cols-[2fr_3fr]"><div className="bg-[#183b35] px-6 py-20 text-[#f6f3eb] sm:px-12 lg:pl-[max(3rem,calc((100vw-1280px)/2+1.5rem))]"><h1 className="max-w-2xl font-display text-[clamp(2.35rem,10vw,3rem)] font-bold leading-[1.06] tracking-tight sm:text-7xl lg:text-[clamp(2.5rem,3.8vw,3.25rem)]">Every child deserves protection. And possibility.</h1><p className="mt-8 max-w-lg text-lg leading-8 text-[#f6f3eb]/80">We are Hope for Vulnerable Children Association. We work with children, families, and communities to protect rights and open opportunities.</p><a href="#mission" className="mt-8 inline-flex min-h-12 items-center gap-3 border-b border-[#f6f3eb]/60 font-semibold">Discover what drives us <ArrowDown aria-hidden="true" className="size-4" /></a></div><div className="relative min-h-96"><Image src="/heros/hero1.webp" alt="Community gathering in support of children" fill priority sizes="(max-width: 1024px) 100vw, 60vw" className="object-cover" /><span aria-hidden="true" className="pointer-events-none absolute inset-x-0 top-0 h-28 bg-gradient-to-b from-[#183b35] to-transparent lg:inset-y-0 lg:left-0 lg:right-auto lg:h-auto lg:w-[42%] lg:bg-gradient-to-r" /></div></section>
