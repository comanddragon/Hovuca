...
export default function AboutPage() {
    return <div className="bg-white text-[var(--brand-forest)]">
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:py-24 lg:grid-cols-[1fr_1.5fr]"><div><h2 className="font-display text-4xl font-bold sm:text-5xl">What we work towards</h2><p className="mt-5 max-w-md leading-7 text-[var(--brand-body-muted)]">Our objectives connect children’s rights with practical care, stronger communities, and shared action.</p><Link href="/projects" className="mt-6 inline-flex min-h-12 items-center gap-3 border-b border-[var(--brand-forest)] font-semibold">Explore our projects <ArrowRight aria-hidden="true" className="size-4" /></Link></div><ul className="border-t border-[var(--brand-forest)]/25">{objectives.map(objective => <li key={objective} className="border-b border-[var(--brand-forest)]/25 py-5 text-lg leading-8">{objective}</li>)}</ul></section>
        <section className="mx-auto max-w-7xl px-6 pb-20"><div className="border-t border-[var(--brand-forest)]/25 pt-10"><h2 className="font-display text-4xl font-bold">Help us protect every child.</h2><p className="mt-5 max-w-xl leading-7 text-[var(--brand-body-muted)]">Join our community of partners, volunteers, and advocates working to create a safer, fairer world for children.</p><div className="mt-8 flex flex-wrap gap-4"><Link href="/volunteers" className="inline-flex min-h-12 items-center bg-[var(--brand-forest)] px-6 py-3 font-semibold text-white hover:bg-[var(--brand-forest-deep)]">Become a volunteer</Link><Link href="/contact" className="inline-flex min-h-12 items-center border border-[var(--brand-forest)] px-6 py-3 font-semibold hover:bg-[var(--brand-sage-light)]">Partner with us</Link></div></div></section>
    </div>;
}
