        ...
        <div className="bg-background text-[var(--brand-forest)]">
            ...
            <section aria-labelledby="volunteer-heading" className="grid lg:min-h-[620px] lg:grid-cols-[0.9fr_1.1fr]">
                <div className="flex flex-col justify-center px-6 py-14 sm:px-10 lg:px-[max(2.5rem,calc((100vw-1280px)/2))] lg:pr-12 lg:py-20">
                    ...
                    <p className="mt-7 max-w-lg text-lg leading-8 text-[var(--brand-body-muted)]">Volunteer with HOVUCA and contribute to community-led work with children, girls and young people in Cameroon.</p>
                    <div className="mt-9 flex flex-wrap items-center gap-5">
                        <a href="#volunteer-profile" className="inline-flex min-h-12 items-center gap-6 bg-[var(--brand-coral)] px-6 py-3 font-semibold text-white transition-colors hover:bg-[var(--brand-coral-dark)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-forest)]">Get involved <ArrowRight aria-hidden="true" className="size-5" /></a>
                        <Link href="/programs" className="py-3 font-semibold underline decoration-[var(--brand-gold)] underline-offset-8 hover:text-[var(--brand-coral-dark)]">Explore our work</Link>
                    </div>
                </div>
            ...
            <section aria-labelledby="contribute-heading" className="mx-auto max-w-7xl px-6 py-16 sm:px-10 lg:py-24">
                <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
                    <div>
                        ...
                        <p className="mt-6 max-w-md leading-7 text-[var(--brand-body-muted)]">Start with what you know and what matters to you. These are areas to discuss with our team, rather than a list of currently open positions.</p>
                    </div>
                    <div className="border-t border-[var(--brand-forest)]/20">
                        {contributions.map((item) => <div key={item.title} className="border-b border-[var(--brand-forest)]/20 py-7"><h3 className="font-display text-2xl font-bold">{item.title}</h3><p className="mt-3 max-w-xl leading-7 text-[var(--brand-body-muted)]">{item.description}</p></div>)}
                    </div>
                </div>
            </section>

            <section aria-labelledby="approach-heading" className="bg-[var(--brand-forest)] text-white">
            ...
            <section id="volunteer-profile" aria-labelledby="profile-heading" className="mx-auto grid max-w-7xl scroll-mt-28 gap-10 px-6 py-16 sm:px-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:py-24">
                <div>
                    ...
                    <p className="mt-6 max-w-md leading-7 text-[var(--brand-body-muted)]">Fill in the volunteer application and tell us what you can offer. No account is required. The team will have your information available for review.</p>
                    <ol className="mt-8 list-decimal space-y-4 pl-5 leading-7 text-[var(--brand-body-muted)]"><li>Enter your contact information.</li><li>Share your skills, interests and availability.</li><li>Submit your application for review.</li></ol>
                    <Link href="/contact" className="mt-8 inline-flex min-h-11 items-center gap-3 font-semibold underline underline-offset-4 hover:text-[var(--brand-coral-dark)]">Talk to our team <ArrowRight aria-hidden="true" className="size-4" /></Link>
                </div>
                <VolunteerApplication />
            </section>

            <section aria-labelledby="questions-heading" className="border-t border-[var(--brand-forest)]/20 bg-[var(--brand-paper)]">
                <div className="mx-auto grid max-w-7xl gap-8 px-6 py-14 sm:px-10 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20 lg:py-20">
                    ...
                    <div>{questions.map((item) => <details key={item.question} className="group border-b border-[var(--brand-forest)]/20 first:border-t"><summary className="cursor-pointer py-5 pr-4 text-base font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--brand-forest)]">{item.question}</summary><p className="pb-6 leading-7 text-[var(--brand-body-muted)]">{item.answer}</p></details>)}</div>
                </div>
            </section>
        </div>
    );
