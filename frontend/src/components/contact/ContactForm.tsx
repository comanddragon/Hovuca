...
const input = "mt-2 w-full border border-[#183b35]/30 bg-white px-4 py-3 font-normal focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#183b35] aria-invalid:border-red-600";
const button = "min-h-12 bg-[#183b35] px-6 py-3 font-semibold text-white hover:bg-[#102c28] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#183b35] disabled:opacity-60";
...
export default function ContactForm() {
    ...
    return <form noValidate onSubmit={handleSubmit(values => submission.mutate(values))}><fieldset disabled={submission.isPending} className="space-y-6">
        <legend className="font-display text-3xl font-bold">Send us a message</legend><p className="text-sm leading-6 text-[#53645f]">All fields are required unless marked optional.</p>
        ...
        <p className="text-sm leading-6 text-[#53645f]">Please do not include passwords, payment PINs, or card details.</p>
        <div><label htmlFor="contact-consent" className="flex items-start gap-3 text-sm leading-6"><input id="contact-consent" type="checkbox" {...register("contact_consent")} {...attributes("contact_consent")} className="mt-1 size-4 shrink-0 accent-[#183b35]" /><span>I agree that HOVUCA may use my information to respond to this enquiry.</span></label>{error("contact_consent")}</div>
