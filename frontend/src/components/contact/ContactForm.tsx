...
const input = "mt-2 w-full border border-[var(--brand-forest)]/30 bg-white px-4 py-3 font-normal focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--brand-forest)] aria-invalid:border-red-600";
const button = "min-h-12 bg-[var(--brand-forest)] px-6 py-3 font-semibold text-white hover:bg-[var(--brand-forest-deep)] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--brand-forest)] disabled:opacity-60";
...
export default function ContactForm() {
    ...
    const error = (field: keyof FormValues) => errors[field] && <span id={`contact-${field}-error`} role="alert" className="mt-2 block text-sm font-normal text-red-800">{errors[field]?.message}</span>;
    if (submission.isSuccess) return <div role="status" className="border-y border-[var(--brand-forest)]/20 py-10"><h3 className="font-display text-3xl font-bold">Your message is received.</h3><p className="mt-4 leading-7 text-[var(--brand-body-muted)]">Thank you for getting in touch. Your enquiry has been saved for the HOVUCA team to review and respond using your contact details.</p><button className={`${button} mt-6`} onClick={() => { reset(); submission.reset(); }}>Send another enquiry</button></div>;
    return <form noValidate onSubmit={handleSubmit(values => submission.mutate(values))}><fieldset disabled={submission.isPending} className="space-y-6">
        <legend className="font-display text-3xl font-bold">Send us a message</legend><p className="text-sm leading-6 text-[var(--brand-body-muted)]">All fields are required unless marked optional.</p>
        ...
        <p className="text-sm leading-6 text-[var(--brand-body-muted)]">Please do not include passwords, payment PINs, or card details.</p>
        <div><label htmlFor="contact-consent" className="flex items-start gap-3 text-sm leading-6"><input id="contact-consent" type="checkbox" {...register("contact_consent")} {...attributes("contact_consent")} className="mt-1 size-4 shrink-0 accent-[var(--brand-forest)]" /><span>I agree that HOVUCA may use my information to respond to this enquiry.</span></label>{error("contact_consent")}</div>
