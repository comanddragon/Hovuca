...
const action = "inline-flex min-h-12 items-center justify-center gap-3 border border-[#183b35]/40 px-5 py-3 text-sm font-semibold hover:bg-[#183b35] hover:text-white focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#183b35]";
export default function DocumentsLibrary() {
    ...
    return <section aria-labelledby="library-heading" className="mx-auto max-w-7xl px-6 py-12 md:py-16">
        ...
        <div className="mt-8 grid gap-5 border-y border-[#183b35]/20 py-6 sm:grid-cols-[2fr_1fr_auto]">
        ...
        <p aria-live="polite" className="py-6 text-sm text-[#53645f]">{isLoading ? "Loading documents…" : isFetching ? "Updating documents…" : isError ? "The library is temporarily unavailable." : `${filtered.length} document${filtered.length === 1 ? "" : "s"} found`}</p>
        {isError ? <div role="alert" className="border-y border-[#183b35]/20 py-10"><h3 className="font-display text-2xl font-bold">We couldn’t load the library.</h3><p className="mt-3 text-[#53645f]">Please try again to access the documents.</p><button disabled={isFetching} className={`${action} mt-6 disabled:opacity-40`} onClick={() => refetch()}>Try again</button></div> : !isLoading && !filtered.length ? <div className="border-y border-[#183b35]/20 py-12"><h3 className="font-display text-2xl font-bold">{query || category ? "No documents match these filters." : "Documents will appear here when published."}</h3>{(query || category) && <button className={`${action} mt-6`} onClick={clear}>Show all documents</button>}</div> : <div aria-busy={isFetching} className="divide-y divide-[#183b35]/20 border-y border-[#183b35]/20">{visible.map(document => <article key={document.id} className="grid gap-6 py-8 sm:grid-cols-[9rem_1fr] lg:grid-cols-[9rem_1fr_auto] lg:items-center">
            <DocumentPreview url={document.file_url} title={document.title} />
            <div><h3 className="font-display text-2xl font-bold leading-tight sm:text-3xl">{document.title}</h3><div className="mt-3 flex flex-wrap gap-4 text-sm text-[#53645f]">{document.category && <span>{document.category}</span>}{document.published_at && <time dateTime={document.published_at}>{new Date(document.published_at).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" })}</time>}</div>{document.description && <p className="mt-4 max-w-3xl leading-7 text-[#53645f]">{document.description}</p>}</div>
            <div className="flex flex-wrap gap-3 sm:col-start-2 lg:col-start-auto lg:flex-col">
                <a href={document.file_url} target="_blank" rel="noopener noreferrer" className={`${action} bg-[#183b35] text-white`}>Open<ExternalLink aria-hidden="true" className="size-4" /><span className="sr-only"> {document.title} in a new tab</span></a>
