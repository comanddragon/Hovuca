...
const field = "mt-2 min-h-12 w-full border border-[var(--brand-forest)]/30 bg-white px-4 py-3 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--brand-forest)]";
const action = "min-h-11 rounded-lg border border-[var(--brand-forest)]/40 px-5 py-2.5 text-sm font-semibold hover:bg-[var(--brand-forest)] hover:text-white disabled:cursor-not-allowed disabled:opacity-40";
const PAGE_SIZE = 6;
export default function ProjectsDirectory() {
    ...
    return <section id="project-directory" className="mx-auto max-w-7xl scroll-mt-28 px-6 py-16 md:py-20"><h2 className="font-display text-3xl font-bold sm:text-4xl">Explore our projects</h2>
        <div className="mt-8 grid gap-5 border-y border-[var(--brand-forest)]/20 py-6 sm:grid-cols-[2fr_1fr_auto]"><label htmlFor="project-search" className="text-sm font-semibold">Search projects<input id="project-search" type="search" value={search} placeholder="Search by title or topic" onChange={event => { setSearch(event.target.value); setPage(1); }} className={field} /></label><label htmlFor="project-status" className="text-sm font-semibold">Project status<select id="project-status" value={status} onChange={event => { setStatus(event.target.value); setPage(1); }} className={field}><option value="">All statuses</option>{Object.entries(statuses).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><button onClick={reset} className={`${action} self-end`}>Clear filters</button></div>
        <div aria-live="polite" className="py-6 text-sm text-[var(--brand-body-muted)]">{isLoading ? "Loading projects…" : isFetching ? "Updating projects…" : !isError ? `${data?.count ?? 0} project${data?.count === 1 ? "" : "s"} found` : "Projects are temporarily unavailable."}</div>
        {isError ? <div role="alert" className="border-y border-[var(--brand-forest)]/20 py-10"><h3 className="font-display text-2xl font-bold">We couldn’t load the projects.</h3><p className="mt-3 text-[var(--brand-body-muted)]">Please try again to see the latest projects.</p><button className={`${action} mt-6`} onClick={() => refetch()} disabled={isFetching}>Try again</button></div> : !isLoading && !data?.results.length ? <div className="border-y border-[var(--brand-forest)]/20 py-12"><h3 className="font-display text-2xl font-bold">{search || status ? "No projects match these filters." : "Projects will appear here when published."}</h3>{(search || status) && <button onClick={reset} className={`${action} mt-6`}>Show all projects</button>}</div> : <div aria-busy={isFetching} className="grid gap-x-8 gap-y-10 md:grid-cols-2">{data?.results.map(project => <article key={project.id} className="border-b border-[var(--brand-forest)]/20 pb-8"><Link href={`/projects/${project.slug}`} className="group block"><div className="relative aspect-[3/2] overflow-hidden rounded-xl bg-[var(--brand-sage)]">{project.cover_image ? <Image src={project.cover_image} alt={project.cover_image_alt || project.title} fill sizes="(max-width: 768px) 100vw, 600px" className="object-cover transition-transform duration-500 motion-safe:group-hover:scale-[1.03]" /> : <div className="flex h-full items-end p-8"><p className="font-display text-3xl text-[var(--brand-forest)]/60">HOVUCA<br />Projects</p></div>}</div><div className="mt-5 flex flex-wrap justify-between gap-3 text-sm text-[var(--brand-body-muted)]"><span>{statuses[project.status]}</span>{project.start_date && <span>{formatDate(project.start_date)}</span>}</div><h3 className="mt-3 flex items-start justify-between gap-4 font-display text-2xl font-bold leading-tight group-hover:underline group-hover:underline-offset-4">{project.title}<ArrowUpRight aria-hidden="true" className="mt-1 size-5 shrink-0" /></h3><p className="mt-4 line-clamp-3 leading-7 text-[var(--brand-body-muted)]">{project.excerpt || project.description}</p></Link></article>)}</div>}
