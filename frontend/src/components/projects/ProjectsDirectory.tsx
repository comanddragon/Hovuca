...
const action = "min-h-11 rounded-lg border border-[#183b35]/40 px-5 py-2.5 text-sm font-semibold hover:bg-[#183b35] hover:text-white disabled:cursor-not-allowed disabled:opacity-40";
const PAGE_SIZE = 6;
export default function ProjectsDirectory() {
    ...
    return <section id="project-directory" className="mx-auto max-w-7xl scroll-mt-28 px-6 py-16 md:py-20"><h2 className="font-display text-3xl font-bold sm:text-4xl">Explore our projects</h2>
        ...
        {isError ? <div role="alert" className="border-y border-[#183b35]/20 py-10"><h3 className="font-display text-2xl font-bold">We couldn’t load the projects.</h3><p className="mt-3 text-[#53645f]">Please try again to see the latest projects.</p><button className={`${action} mt-6`} onClick={() => refetch()} disabled={isFetching}>Try again</button></div> : !isLoading && !data?.results.length ? <div className="border-y border-[#183b35]/20 py-12"><h3 className="font-display text-2xl font-bold">{search || status ? "No projects match these filters." : "Projects will appear here when published."}</h3>{(search || status) && <button onClick={reset} className={`${action} mt-6`}>Show all projects</button>}</div> : <div aria-busy={isFetching} className="grid gap-x-8 gap-y-10 md:grid-cols-2">{data?.results.map(project => <article key={project.id} className="border-b border-[#183b35]/20 pb-8"><Link href={`/projects/${project.slug}`} className="group block"><div className="relative aspect-[3/2] overflow-hidden rounded-xl bg-[#e3e8dd]">{project.cover_image ? <Image src={project.cover_image} alt={project.cover_image_alt || project.title} fill sizes="(max-width: 768px) 100vw, 600px" className="object-cover transition-transform duration-500 motion-safe:group-hover:scale-[1.03]" /> : <div className="flex h-full items-end p-8"><p className="font-display text-3xl text-[#183b35]/60">HOVUCA<br />Projects</p></div>}</div><div className="mt-5 flex flex-wrap justify-between gap-3 text-sm text-[#53645f]"><span>{statuses[project.status]}</span>{project.start_date && <span>{formatDate(project.start_date)}</span>}</div><h3 className="mt-3 flex items-start justify-between gap-4 font-display text-2xl font-bold leading-tight group-hover:underline group-hover:underline-offset-4">{project.title}<ArrowUpRight aria-hidden="true" className="mt-1 size-5 shrink-0" /></h3><p className="mt-4 line-clamp-3 leading-7 text-[#53645f]">{project.excerpt || project.description}</p></Link></article>)}</div>}
