...
const fieldClass = "mt-2 w-full border border-[#183b35]/30 bg-white px-4 py-3 font-normal text-[#183b35] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#183b35] aria-invalid:border-red-600";
...
export default function VolunteerApplication() {
    ...
    if (submission.isSuccess) return <div role="status" className="border-y border-[#183b35]/20 py-10">
        <CheckCircle2 aria-hidden="true" className="size-10 text-[#183b35]" />
        <h3 className="mt-5 font-display text-3xl font-bold">Application received.</h3>
        <p className="mt-4 leading-7 text-[#53645f]">Thank you for offering your time and skills. Your application has been submitted for review. The team can use your contact details to discuss next steps.</p>
        <p className="mt-4 text-sm text-[#53645f]">Submitting an application does not confirm a volunteer placement.</p>
    </div>;

    return <form onSubmit={handleSubmit((values) => submission.mutate(values))} noValidate className="border-t border-[#183b35]/20 pt-8">
        <fieldset disabled={submission.isPending} className="space-y-6">
            ...
            <p className="text-sm leading-6 text-[#53645f]">Tell us about yourself and how you would like to contribute. All fields are required unless marked optional.</p>
            ...
            <label className="block font-semibold" htmlFor="occupation">Occupation or studies <span className="font-normal text-[#53645f]">(optional)</span><input id="occupation" maxLength={200} {...register("occupation")} {...accessibility("occupation")} className={fieldClass} />{errorFor("occupation")}</label>
