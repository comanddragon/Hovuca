...
const fieldClass = "mt-2 w-full border border-primary/30 bg-brand-white px-4 py-3 font-normal text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary aria-invalid:border-brand-error";
const actionClass = "inline-flex min-h-12 items-center justify-center gap-3 bg-primary px-6 py-3 font-semibold text-brand-white transition-colors hover:bg-brand-forest-deep focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary disabled:cursor-not-allowed disabled:opacity-60";
...
export default function VolunteerApplication() {
    ...
    const errorFor = (field: keyof ApplicationForm) => errors[field]
        ? <span id={`${field}-error`} role="alert" className="mt-2 block text-sm font-normal text-brand-error">{errors[field]?.message}</span>
        : null;
    ...
    if (submission.isSuccess) return <div role="status" className="border-y border-primary/20 py-10">
        ...
        <p className="mt-4 leading-7 text-muted-foreground">Thank you for offering your time and skills. Your application has been submitted for review. The team can use your contact details to discuss next steps.</p>
        <p className="mt-4 text-sm text-muted-foreground">Submitting an application does not confirm a volunteer placement.</p>
    </div>;

    return <form onSubmit={handleSubmit((values) => submission.mutate(values))} noValidate className="border-t border-primary/20 pt-8">
        <fieldset disabled={submission.isPending} className="space-y-6">
            ...
            <p className="text-sm leading-6 text-muted-foreground">Tell us about yourself and how you would like to contribute. All fields are required unless marked optional.</p>
            ...
            <label className="block font-semibold" htmlFor="occupation">Occupation or studies <span className="font-normal text-muted-foreground">(optional)</span><input id="occupation" maxLength={200} {...register("occupation")} {...accessibility("occupation")} className={fieldClass} />{errorFor("occupation")}</label>
